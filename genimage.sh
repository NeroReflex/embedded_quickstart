...
if [ -f "${BINARIES_DIR}/imx-boot" ]; then
    ...
    if [ $ROOTFS_PARTUUID_RESULT -eq 0 ]; then
        ...
        echo "    options \"root=PARTUUID=$ROOTFS_PARTUUID ro rootfstype=btrfs rootdelay=2 video=efifb:1920x1080\" add_efi_memmap" >> "${TARGET_ROOTFS}/EFI/refind/refind.conf"
        echo "    #options \"root=PARTLABEL=rootfs ro rootfstype=btrfs rootdelay=2 video=efifb:1920x1080\"" >> "${TARGET_ROOTFS}/EFI/refind/refind.conf"
