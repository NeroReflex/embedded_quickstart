...
echo 'backend=drm,vnc' >> "${WESTON_INI}"
echo 'idle-time=0' >> "${WESTON_INI}"
echo '' >> "${WESTON_INI}"
echo '[vnc]' >> "${WESTON_INI}"
echo 'refresh-rate=15' >> "${WESTON_INI}"
echo "tls-key=/etc/certs/$AUTOLOGIN_USERNAME/tls.key" >> "${WESTON_INI}"
echo "tls-cert=/etc/certs/$AUTOLOGIN_USERNAME/tls.crt" >> "${WESTON_INI}"
echo '' >> "${WESTON_INI}"
echo '' >> "${WESTON_INI}"
echo '[output]' >> "${WESTON_INI}"
echo 'name=LVDS-1' >> "${WESTON_INI}"
echo '' >> "${WESTON_INI}"
echo '[output]' >> "${WESTON_INI}"
echo 'name=vnc' >> "${WESTON_INI}"
echo 'mirror-of=LVDS-1' >> "${WESTON_INI}"
echo '' >> "${WESTON_INI}"
...
mkdir -p /etc/certs/$AUTOLOGIN_USERNAME
chown $AUTOLOGIN_UID:$AUTOLOGIN_GID /etc/certs/$AUTOLOGIN_USERNAME
sudo -u $AUTOLOGIN_USERNAME openssl genrsa -out "/etc/certs/$AUTOLOGIN_USERNAME/tls.key" 2048
sudo -u $AUTOLOGIN_USERNAME openssl req -new -key "/etc/certs/$AUTOLOGIN_USERNAME/tls.key" -out "/etc/certs/$AUTOLOGIN_USERNAME/tls.csr" -subj "/C=IT/ST=Veneto/L=Mestrino/O=MITEC Elettronica s.r.l./OU=SE/CN=mitec.it"
sudo -u $AUTOLOGIN_USERNAME openssl x509 -req -days 36500 -signkey "/etc/certs/$AUTOLOGIN_USERNAME/tls.key" -in "/etc/certs/$AUTOLOGIN_USERNAME/tls.csr" -out "/etc/certs/$AUTOLOGIN_USERNAME/tls.crt"
rm "/etc/certs/$AUTOLOGIN_USERNAME/tls.csr"
