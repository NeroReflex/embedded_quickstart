...
overlay       /mnt     overlay     x-initrd.mount,defaults,x-systemd.requires-mounts-for=/base,lowerdir=/mnt,upperdir=/base/${DEPLOYMENTS_DATA_DIR}/${DEPLOYMENT_SUBVOL_NAME}/mnt_overlay/upperdir,workdir=/base/${DEPLOYMENTS_DATA_DIR}/${DEPLOYMENT_SUBVOL_NAME}/mnt_overlay/workdir,index=off,metacopy=off,xino=off,redirect_dir=off                       0   0
